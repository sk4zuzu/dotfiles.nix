{ config, pkgs, lib, ... }: {
  ...
  environment.systemPackages = with pkgs; [
    alacritty
    bat bc bridge-utils
    cabextract cdrkit chromium cpufrequtils cryptsetup
    dmidecode dnsutils dos2unix
    edid-decode efibootmgr ethtool exfat
    fd feh ffmpeg file
    gimp git gnumake gnuplot gnupg gptfdisk
    htop hsetroot
    igrep iptables iotop
    jq
    lm_sensors lsof
    maim mc mesa-demos mkpasswd mpv multipath-tools mupdf
    neovim nftables nmap ntfs3g ntp
    openssl openvpn
    p7zip patch pavucontrol pciutils pv
    radeontop remmina ripgrep rsync
    sdparm syslinux
    tcpdump tmux transmission_4-gtk
    unrar unshield unzip usbutils
    vim
    (wine.override { wineRelease = "staging"; }) winetricks wireguard-tools
    xclip xorg.xdpyinfo xorg.xev xorg.xgamma xorg.xhost xorg.xkill xorg.xmodmap
    yt-dlp
    zip
  ];
